...
export class FXPBuyGuard extends SmartContract {
    constructor() {
        super(...arguments)
    }
    ...
    public redeem(
        ...
        assert(
            preTx.outputScriptList[Number(2)] == serviceFeeScript ||
                preTx.outputScriptList[Number(3)] == serviceFeeScript,
            'should pay service fee address'
        )
        assert(
            preTx.outputSatoshisList[Number(2)] == serviceFeeSats ||
                preTx.outputSatoshisList[Number(3)] == serviceFeeSats,
